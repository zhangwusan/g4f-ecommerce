...
export class IngredientService {
  ...
  async get(query: BaseQueryDto) {
    try {
      ...
      const { rows: data, count } = await Ingredient.findAndCountAll({
  ...
  async create(body: CreateIngredientDto, creator: UserDecoratorType) {
  ...
  async update(id: number, body: CreateIngredientDto, updater: UserDecoratorType) {
    const sequelize = new Sequelize(sequelizeConfig);
    const transaction = await sequelize.transaction();
    try {
      const ingredient = await Ingredient.findByPk(id);
  ...
  async delete(id: number, deleter: UserDecoratorType) {
    ...
    try {
      ...
      await ingredient.update({ deleter_id: deleter.id });
