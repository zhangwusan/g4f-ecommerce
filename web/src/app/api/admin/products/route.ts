...
import { CreateProductPayload, ProductManagementDisplay } from "@/lib/type/product.interface";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const apiUrl = `${apiBaseUrlV1}/products/management${query.toString() ? `?${query.toString()}` : ''}`;
    ...
    const { data, pagination, headers }: { data: ProductManagementDisplay[], pagination: Pagination, headers: HeadersDisplay[] } = await response.json();
...
export async function POST(req: NextRequest) {
  try {
    ...
    const apiUrl = `${apiBaseUrlV1}/products/management`;
    const response = await fetchWithToken(apiUrl, {
      method: 'POST',
