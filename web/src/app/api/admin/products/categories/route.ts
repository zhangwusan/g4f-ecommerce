...
export async function GET(request: NextRequest) {
    try {
        ...
        const url = new URL(`${apiBaseUrlV1}/categories/all-categories`);
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log("POST /categories payload:", payload);

        const url = new URL(`${apiBaseUrlV1}/categories`);
        ...
        if (!response.ok) {
            return NextResponse.json({ error: 'Failed to add category' }, { status: response.status });
        }
